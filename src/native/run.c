...
int main() {
    ...
    for(int i = 0; i < HARD_COUNT; ++i) {
        krillcoin_hard_hash(out, in, 5, 512);
        in[0]++;
    }
    ...
    for(int i = 0; i < HARD_COUNT; ++i) {
        krillcoin_hard_hash(out, in, 5, 1024);
        in[0]++;
    }
    ...
    for(int i = 1; i < 4; ++i) {
        ...
        uint32_t nonce = krillcoin_hard_hash_target(out, in, strlen(in), 0x20000000u + (0xffff >> i), 0, (uint32_t)-1, 512);
